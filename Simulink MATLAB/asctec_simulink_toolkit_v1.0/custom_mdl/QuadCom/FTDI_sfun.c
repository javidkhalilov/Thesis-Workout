...
#define S_FUNCTION_NAME FTDI_sfun
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
/* Output Port  3 */
#define OUT_PORT_3_NAME      bytes_sent
...
extern void FTDI_sfun_Outputs_wrapper(const int32_T *BaudRate,
...
{
    const int32_T   *BaudRate  = (const int32_T*) ssGetInputPortSignal(S,0);
    ...
    int16_T        *bytes_sent  = (int16_T *)ssGetOutputPortRealSignal(S,3);

    FTDI_sfun_Outputs_wrapper(BaudRate, data_send, data_out, data_new_flag, HL_Status, bytes_sent);
}
